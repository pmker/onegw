...
import (
	...
	"github.com/pmker/onegw/oneplus/common/kvstore"
...
func (db *BadgerDB) Set(k, v string, ttl int) error {
	return db.badger.Update(func(txn *badger.Txn) (err error) {
		if ttl < 1 {
			...
			e := badger.NewEntry([]byte(k), []byte(v)) //.WithTTL(duration)
...
func (db *BadgerDB) Has(k string) bool {
	...
	db.badger.View(func(txn *badger.Txn) error { //db.View相当于打开了一个读写事务:db.NewTransaction(true)。用db.Update的好处在于不用显式调用Txn.Discard()了
