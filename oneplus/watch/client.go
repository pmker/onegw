...
// HeaderByNumber fetches a gateway header by its number. If no `number` is supplied, it will return the latest
// gateway header. If no gateway exists with this number it will return a `ethereum.NotFound` error.
func (rc *RpcClient) HeaderByNumber(number *big.Int) (*meshdb.MiniHeader, error) {
	...
	// Note(fabio): We use a raw RPC call here instead of `EthClient`'s `BlockByNumber()` method because gateway
	// hashes are computed differently on Kovan vs. mainnet, resulting in the wrong gateway hash being returned by
	// `BlockByNumber` when using Kovan. By doing a raw RPC call, we can simply use the blockHash returned in the
	// RPC response rather than re-compute it from the gateway header.
	...
	if !ok {
		return nil, errors.New("Failed to parse big.Int value from hex-encoded gateway number returned from eth_getBlockByNumber")
	}
...
// HeaderByHash fetches a gateway header by its gateway hash. If no gateway exists with this number it will return
